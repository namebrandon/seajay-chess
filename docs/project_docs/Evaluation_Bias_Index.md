...
- 2025-10-12: SEE now applies a `CHECK_FOLLOWUP_BONUS` of 60 cp to captures that give an immediate check, keeping queen/slider sacs from being tossed out purely on material loss while we shift focus to search heuristics (`src/core/see.h:39`, `src/core/see.cpp:500-505`). SPRT vs `main` (~3.7k games) is neutral, so this is our pause point.
- 2025-10-12: SearchData telemetry for the queen-sack harness captured at depth 12 with `searchdata_r1b1k2r_depth12.log` and `searchdata_wac049_depth12.log` (`docs/project_docs/telemetry/queen_sack/2025-10-07/`). Logs show TT cutoffs remaining dominant (48–63 %), counter-history usage ≈60 %, and queen-check lines still failing to reach the front of the move list, so QS2 ordering work remains open.
- 2025-10-12 (QS2 follow-up): Counter-history throttled when queen contact captures are present, contact replays now persist after fail-lows, and replayed captures preempt TT moves. Fresh telemetry (`searchdata_r1b1k2r_depth12_after_qs2c.log`, `searchdata_wac049_depth12_after_qs2c.log`, `tactical_queen-sack_2025-10-12_after_qs2c.csv`) shows the 200 ms suite inching to 5/20 while key sacs remain evaluation-bound. A 500 ms rerun (`tactical_queen-sack_2025-10-12_after_qs2c_500ms.csv`) hits 9/20, confirming ordering fixes extend deeper but evaluation gaps still block the remaining motifs.
- 2025-10-11: QS3 danger detection now treats king-only defenses as unsafe. Static evals on the six worst motifs jump by 150–400 cp (`docs/project_docs/telemetry/eval_bias/eval_extended_r3r1k1.txt`), but depth‑18 search still prefers quiet continuations, so follow-up work moves to search selectivity and move ordering. Attempting to extend QS3 to slider contacts plus attacker rebates produced a ~−95 nELO regression and has been rolled back; future experiments should target search heuristics first and gate static deltas behind proven ordering improvements.
...
- 2025-10-11: Remaining regression cases are search-bound. Depth-18 search on `r3r1k1/pp1q1pp1/...` still outputs −112 cp even though the static eval is +190 cp (`docs/project_docs/telemetry/eval_bias/eval_extended_r3r1k1.txt`), indicating follow-up work is needed on move ordering / pruning gates to capitalise on the stronger heuristic signal. Attempting to extend QS3 to slider contacts plus attacker rebates produced a ~−95 nELO regression and has been rolled back; future experiments should target search heuristics first and gate static deltas behind proven ordering improvements.
...
- `docs/project_docs/telemetry/queen_sack/2025-10-07/searchdata_r1b1k2r_depth12.log` / `searchdata_wac049_depth12.log` – depth‑12 SearchData snapshots confirming contact-check moves still fall behind TT/first-capture pruning despite SEE bonuses.
- `docs/project_docs/telemetry/queen_sack/2025-10-07/searchdata_r1b1k2r_depth12_after_qs2c.log` / `searchdata_wac049_depth12_after_qs2c.log` – counter-history throttled; TT dominance remains but queen-contact replays now sit at the front of the move list.
- `docs/project_docs/telemetry/queen_sack/2025-10-07/tactical_queen-sack_2025-10-12_after_qs2c.csv` – latest 200 ms sweep with the QS2 replay/throttle adjustments (5/20 hits).
- `docs/project_docs/telemetry/queen_sack/2025-10-07/tactical_queen-sack_2025-10-12_after_qs2c_500ms.csv` – extended 500 ms sweep (9/20 hits) plus auto-generated summaries `tools/tactical_test_2025-10-12_21-17-33.csv` / `tools/tactical_failures_2025-10-12_21-17-33.csv`.
