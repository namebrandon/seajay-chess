...
- 2025-10-03: `feature/eval-framework/pawn-loose-pawns` introduces "loose pawn" penalties when forward pawns lack friendly pawn defenders. Initial defaults (`EvalLoosePawnEnemyHalfPenalty=6`, `EvalLoosePawnOwnHalfPenalty=2`, `EvalLoosePawnPhalanxRebate=3`) only fire when the pawn isn't part of a phalanx, keeping the surcharge lightweight until SPSA rebalances with the other pawn branches.
- 2025-09-30: SPSA batch 2 promotes the pawn-span knobs to new defaults (`EvalLoosePawnOwnHalfPenalty=6`, `EvalLoosePawnEnemyHalfPenalty=13`, `EvalPawnInfiltrationBonus=25`, `EvalPawnTensionPenalty=3`, `EvalPawnPushThreatBonus=6`) ahead of the SPRT validation pass.
- 2025-10-05: SPRT 726 (`integration/eval-framework` vs `main`, EvalPasserPhaseP4=true) hits +12.5 ± 7.0 nELO (LLR 3.07 in [−2.94, 2.94]); promote `EvalPasserPhaseP4` to default ON and keep the toggle exposed for future tuning.
...
- 2025-10-03: `feature/eval-framework/pawn-bishop-color` adds bishop/pawn color-complex scoring with telemetry (harmony, tension, blocked central same-color counts) and exposes SPSA knobs (`EvalBishopColorHarmonyBonus=2`, `EvalBishopColorTensionPenalty=2`, `EvalBishopColorBlockedPenalty=3`). New EvalExtended detail block `bishop_color` surfaces light/dark pawn splits for tuning. OpenBench test 714 vs. `integration/eval-framework` (EvalPasserPhaseP4 enabled) returned +0.29 ± 4.71 nELO (LLR −0.47 in [−2.94, 2.94]); treat as neutral and keep defaults ahead of combined SPSA.
- 2025-10-04: `feature/eval-framework/pawn-span-tension` introduces pawn span-driven infiltration, tension, and push-threat scoring with knobs (`EvalPawnInfiltrationBonus=4`, `EvalPawnTensionPenalty=2`, `EvalPawnPushThreatBonus=3`) plus EvalExtended detail `pawn_span`. Telemetry pack `eval_20250929T194205Z.json` captured post-implementation; SPRT pending.
...
6. **`feature/eval-framework/pawn-bishop-color` – Bishop/pawn color complexes:** Telemetry-aware scoring implemented on feature branch with default knobs (`HarmonyBonus=2`, `TensionPenalty=2`, `BlockedPenalty=3`). Pawn pack telemetry archived (`eval_20250929T190050Z.json`), SPRT 714 neutral; next step is to include these knobs in the consolidated SPSA batch after pending pawn-stage code lands.
7. **`feature/eval-framework/pawn-span-tension` – Pawn span & tension metrics:** Implementation in progress with scoring + telemetry wired (see `eval_20250929T194205Z.json`). Next steps: validate infiltration heuristics on additional FENs, then schedule SPRT before folding the knobs into the consolidated SPSA batch.
...
- Adopt a danger-index model inspired by modern engines (e.g., Perseus) but implemented from scratch: accumulate per-phase king-danger contributions (ring pressure by piece type, safe-check availability, flank pressure, pinned defenders, pawn shield/storm quality) and map through a non-linear lookup so stacked threats surge while quiet positions stay flat. Include explicit accounting for safe queen checks to address current tactical blind spots (queen captures delivering check) and expose telemetry counters to confirm coverage.
- Derive the danger inputs from telemetry we already gather (multi-attacks, weak squares, pawn spans) to minimize new overhead, and share attack maps with threat/king modules to keep performance aligned with our NPS targets.
- Ensure the new danger index remains SPSA-friendly by gating the final table behind a small set of tunable knobs, and log `EvalExtended` entries so harness diffs can track which signals fired.
- UCI scaffolding landed: `EvalKingDangerIndex` / `EvalKingDangerTelemetry` toggles plus baseline weights (`EvalKingDangerRingWeight`, `EvalKingDangerOuterRingWeight`, `EvalKingDangerSafeCheckWeight`, `EvalKingDangerQueenSafeCheckWeight`, `EvalKingDangerFlankPressureWeight`, `EvalKingDangerPinnedDefenderPenalty`, `EvalKingDangerShieldBonus`, `EvalKingDangerStormPenalty`) now expose the knobs. When the index is enabled it aggregates ring/outer/flank pressure, pinned defenders, shield/storm counts, and safe queen checks; defaults remain off so mainline play is unchanged while we validate telemetry.
- 2025-09-30: WAC tactical sweep (1s/position, telemetry on) confirmed the danger trace captures safe queen checks and pressure metrics; 273/300 pass (91.0% success) with failures logged for follow-up tuning.
- 2025-10-05: Corrected the king-danger score sign so black-danger now boosts White (and vice versa). With the fix, queen-sac motifs like WAC.055/WAC.145 swing from negative scores (−72/−3) to +64/+129 when the index is active, matching the "attacker benefits" expectation before we retune the weights.
- 2025-10-05: Re-ran the full WAC suite at 1s with `EvalKingDangerIndex=true` after the sign fix: 273/300 (91.0%) with 27 residual failures. Logged a per-position breakdown at `docs/project_docs/telemetry/tactical/wac_20250930_king_danger_breakdown.csv`, showing the new flank-multi and storm-tier counters. Queen-check misses now average +68cp from the index but still fall out of the root PV, so the next iteration needs move-ordering support.
- 2025-10-05: Extended king-danger telemetry with flank multi-attack counts and three storm-depth tiers (far/mid/near). The new fields surface in EvalExtended traces only; scoring still uses the original weights until we validate the telemetry on curated packs.
- 2025-10-05: Search hooks now respect those signals: late-ranked captures that deliver check no longer fail SEE gating, and contact checks from queen captures earn a +1 extension when they land in the king’s ring. WAC.049 still fails at 1 s, but the debug trace confirms the new extension fires; next pass will focus on move re-search/historical boosts for the sacrificial line.
- 2025-10-05: Opened a focused tactical triage on the long-term hold-out WAC.049. SeaJay still selects `h5h6` over the book `Qxh7+`; the index credits the attack (+68 cp swing), but the search prefers the slow pawn push because SEE/LMR prune the sacrificial follow-up after `...Kxh7`. Notes and telemetry captures live in `docs/project_docs/telemetry/tactical/wac049_analysis.md` for iteration.
