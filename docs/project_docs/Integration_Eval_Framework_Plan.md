...
3. **`feature/eval-framework/pawn-loose-pawns` – Loose and unsupported pawns:** Implementation underway. Follow-up: capture EvalExtended traces on the pawn pack to confirm unsupported pawns register, then prep threat-module reuse notes before moving to phalanx bonuses.
