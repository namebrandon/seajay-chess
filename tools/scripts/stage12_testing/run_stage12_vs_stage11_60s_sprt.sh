#!/bin/bash
# SPRT Test: Stage 12 (TT) vs Stage 11 (MVV-LVA) - 60 second time control
# Longer time control for more accurate evaluation of TT benefits
# TT effectiveness improves with deeper searches
...
OUTPUT_DIR="$PROJECT_ROOT/sprt_results/SPRT-Stage12-vs-Stage11-60s-$TIMESTAMP"
mkdir -p "$OUTPUT_DIR"
...
# Testing for TT expected gain with longer searches
# TT becomes more effective with deeper searches
...
# Opening book - use 8moves_v3.pgn for variety
OPENING_BOOK="$PROJECT_ROOT/external/books/8moves_v3.pgn"
# Fallback to 4moves if 8moves not available
...
echo "==================================================================="
echo "SPRT Test: Stage 12 (TT) vs Stage 11 (MVV-LVA) - 60s Time Control"
echo "==================================================================="
echo "Testing Transposition Table with longer time control"
echo "TT benefits increase with search depth:"
echo "  - More positions to cache"
echo "  - Better move ordering from deeper searches"
echo "  - Higher hit rates in complex positions"
echo ""
echo "Expected gain: +130-175 Elo (may be higher at 60s)"
echo ""
echo "Engines:"
...
echo ""
echo "Parameters:"
echo "  Time Control: $TIME_CONTROL (LONGER)"
echo "  SPRT: elo0=$ELO0, elo1=$ELO1, alpha=$ALPHA, beta=$BETA"
if [ -n "$OPENING_BOOK" ]; then
    echo "  Opening Book: $(basename $OPENING_BOOK)"
else
...
echo "  - Estimated time: 12-24 hours at 60+0.6 time control"
echo ""
echo "Press Ctrl+C to stop early if needed"
echo ""

# Build fast-chess command
# Fewer rounds due to longer time control
FASTCHESS_CMD="$FASTCHESS \
    -engine name=\"Stage12-TT-60s\" cmd=\"$STAGE12_BINARY\" \
    -engine name=\"Stage11-MVV-LVA-60s\" cmd=\"$STAGE11_BINARY\" \
...
Time Control: $TIME_CONTROL (60 seconds + 0.6 increment)
