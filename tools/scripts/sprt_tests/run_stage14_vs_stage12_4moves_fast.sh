...
TEST_NAME="SPRT-Stage14v12-4moves-fast-${TIMESTAMP}"
...
if [ ! -f "${ENGINE_BASE}" ]; then
    echo "Error: Stage 12 engine not found at ${ENGINE_BASE}"
    exit 1
fi
...
Opening Book: 4moves_test.pgn
Expected Result: H1 accepted (+100-200 Elo improvement)
Features Tested:
  - Quiescence search + Iterative deepening combined
  - Cumulative improvements from Stage 12 to 14
  - Time control fixes (frequent checking, emergency cutoff, increased buffer)
EOF

echo "======================================"
echo "SPRT Test: Stage 14 vs Stage 12"
echo "======================================"
echo "New Engine: Stage 14 (Quiescence Search + ID)"
echo "Base Engine: Stage 12 (Transposition Tables)"
echo "Opening Book: 4moves_test.pgn"
echo "Time Control: ${TIME_CONTROL}"
echo "SPRT Bounds: [${ELO0}, ${ELO1}] ELO"
echo "Hypothesis: Stage 14 should gain 100+ ELO over Stage 12"
...
"${FASTCHESS}" \
    ...
    -engine name="Stage12-TT" cmd="${ENGINE_BASE}" \
...
if [ -f "${RESULT_DIR}/console_output.txt" ]; then
    ...
    if grep -q "H1 accepted" "${RESULT_DIR}/console_output.txt"; then
        echo ""
        echo "✅ SUCCESS: Stage 14 shows significant improvement over Stage 12!"
