#!/bin/bash
# Stage 11 Self-Play Test for White Bias Investigation
# MVV-LVA implementation - Expected: WHITE BIAS PRESENT (if PST bug persists)

echo "================================================"
echo "Stage 11 Self-Play Test - White Bias Investigation"
echo "================================================"
echo ""
echo "Testing: Stage 11 (MVV-LVA Move Ordering) self-play from startpos"
echo "Expected: WHITE BIAS - If PST bug persists, but MVV-LVA may change patterns"
echo "Time control: 10+0.1"
echo ""

# Configuration
ENGINE="/workspace/binaries/seajay-stage11-f2ad9b5"
OUTPUT_DIR="/workspace/tools/scripts/sprt_tests/white_bias_investigation"
LOG_FILE="$OUTPUT_DIR/stage11_selfplay_results.log"
...
if [ ! -f "$ENGINE" ]; then
    echo "ERROR: Stage 11 binary not found: $ENGINE"
    exit 1
fi
...
# Run self-play test
echo "Starting Stage 11 self-play test..."
echo "Running $GAMES games from starting position"
echo "NOTE: MVV-LVA move ordering may affect game patterns and diversity"
echo ""

# Use fast-chess for self-play
"$FASTCHESS" \
    -engine cmd="$ENGINE" name=Stage11-A \
    -engine cmd="$ENGINE" name=Stage11-B \
    -each tc="$TC" \
    -games 2 -rounds 50 -repeat \
    -pgnout file="$OUTPUT_DIR/stage11_selfplay.pgn" \
    -log file="$LOG_FILE" level=info \
    -recover \
    -event "Stage 11 Self-Play White Bias Test" \
    -site "Starting Position Only"

# Extract and display results
echo ""
echo "================================================"
echo "Stage 11 Self-Play Results:"
echo "================================================"
...
if [ -f "$LOG_FILE" ]; then
    ...
    MATCH_LINE=$(grep -E "Score of Stage11-A vs Stage11-B" "$LOG_FILE" | tail -1)
    ...
    if [ $TOTAL -gt 0 ]; then
        ...
        if [ $DIFF -gt 20 ]; then
            ...
            echo "    PST bug likely persists in Stage 11!"
            echo "    MVV-LVA didn't fix the underlying evaluation bias."
        else
            echo ""
            echo "✓ Engines have balanced scores"
            echo "    MVV-LVA move ordering may have affected game diversity."
        fi
    fi
else
...
echo "PGN saved to: $OUTPUT_DIR/stage11_selfplay.pgn"