...
# Configuration
OUTPUT_DIR="/workspace/tools/scripts/sprt_tests/white_bias_investigation/progression_results"
...
# Output files
PGN_FILE="$OUTPUT_DIR/stage9_vs_stage10_$(date +%Y%m%d_%H%M%S).pgn"
LOG_FILE="$OUTPUT_DIR/stage9_vs_stage10_$(date +%Y%m%d_%H%M%S).log"
...
# Run the match
"$FASTCHESS" \
    -engine cmd="$STAGE9" name="Stage9-PST" \
...
if [ -f "$LOG_FILE" ]; then
    # Extract final score
    SCORE_LINE=$(grep -E "Score of Stage9-PST vs Stage10-Magic" "$LOG_FILE" | tail -1)
        ...
        if [[ "$SCORE_LINE" =~ ([0-9]+)[[:space:]]-[[:space:]]([0-9]+)[[:space:]]-[[:space:]]([0-9]+) ]]; then
            ...
            if [ $((S9_WINS + S10_WINS)) -gt 0 ]; then
                ...
                if (( $(echo "$WIN_RATE > 50" | bc -l) )); then
                    ...
                    echo "Stages appear equal in strength"
                fi
            fi
        fi
    fi
    ...
    echo ""
    CRASHES=$(grep -c "loses on time\|crashed\|disconnects" "$LOG_FILE" 2>/dev/null || echo "0")
    if [ "$CRASHES" -gt 0 ]; then
        echo "⚠️  Warning: $CRASHES crashes/timeouts detected"
        grep "loses on time\|crashed\|disconnects" "$LOG_FILE" | head -5
    fi
fi
