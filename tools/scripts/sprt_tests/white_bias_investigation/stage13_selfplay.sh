#!/bin/bash
# Stage 13 Self-Play Test for White Bias Investigation
# Iterative Deepening implementation - Expected: WHITE BIAS PRESENT (if PST bug persists)

echo "================================================"
echo "Stage 13 Self-Play Test - White Bias Investigation"
echo "================================================"
echo ""
echo "Testing: Stage 13 (Iterative Deepening) self-play from startpos"
echo "Expected: WHITE BIAS - If PST bug persists from earlier stages"
echo "Time control: 10+0.1"
echo ""

# Configuration
ENGINE="/workspace/binaries/seajay-stage13-869495e"
OUTPUT_DIR="/workspace/tools/scripts/sprt_tests/white_bias_investigation"
LOG_FILE="$OUTPUT_DIR/stage13_selfplay_results.log"
...
if [ ! -f "$ENGINE" ]; then
    echo "ERROR: Stage 13 binary not found: $ENGINE"
    exit 1
fi
...
# Run self-play test
echo "Starting Stage 13 self-play test..."
echo "Running $GAMES games from starting position"
echo "NOTE: Iterative deepening changes search strategy but not evaluation"
echo ""

# Use fast-chess for self-play
"$FASTCHESS" \
    -engine cmd="$ENGINE" name=Stage13-A \
    -engine cmd="$ENGINE" name=Stage13-B \
    -each tc="$TC" \
    -games 2 -rounds 50 -repeat \
    -pgnout file="$OUTPUT_DIR/stage13_selfplay.pgn" \
    -log file="$LOG_FILE" level=info \
    -recover \
    -event "Stage 13 Self-Play White Bias Test" \
    -site "Starting Position Only"

# Extract and display results
echo ""
echo "================================================"
echo "Stage 13 Self-Play Results:"
echo "================================================"
...
if [ -f "$LOG_FILE" ]; then
    ...
    MATCH_LINE=$(grep -E "Score of Stage13-A vs Stage13-B" "$LOG_FILE" | tail -1)
    ...
    if [ $TOTAL -gt 0 ]; then
        ...
        if [ $DIFF -gt 20 ]; then
            ...
            echo "    PST bug likely persists in Stage 13!"
            echo "    Iterative deepening didn't fix the evaluation bias."
        else
            echo ""
            echo "✓ Engines have balanced scores"
            echo "    Iterative deepening may have improved game quality."
        fi
    fi
else
...
echo "PGN saved to: $OUTPUT_DIR/stage13_selfplay.pgn"