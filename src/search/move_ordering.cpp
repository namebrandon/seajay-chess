...
int MvvLvaOrdering::scoreMove(const Board& board, Move move) noexcept {
    ...
    if (isCapture(move)) {
        ...
        // SIMPLE FIX: Heavily penalize bad captures (heavy pieces taking pawns)
        // This ensures they are searched AFTER killer moves
        if (victim == PAWN && (attacker == QUEEN || attacker == ROOK)) {
            score -= 10000;  // Move to end of capture list
        }
