...
Score KingSafety::evaluate(const Board& board, Color side) {
    ...
    const int enemyMajorCount = popCount(board.pieces(enemy, ROOK)) + popCount(board.pieces(enemy, QUEEN));
    const int enemyMinorCount = popCount(board.pieces(enemy, BISHOP)) + popCount(board.pieces(enemy, KNIGHT));
    ...
    if (attackedRingSquares >= 2) {
        const int extraMgPenalty = attackedRingSquares * (enemyMajorCount * 20 + enemyMinorCount * 8);
        const int extraEgPenalty = attackedRingSquares * (enemyMajorCount * 10 + enemyMinorCount * 4);
        mgScore -= extraMgPenalty;
        egScore -= extraEgPenalty;
    }
