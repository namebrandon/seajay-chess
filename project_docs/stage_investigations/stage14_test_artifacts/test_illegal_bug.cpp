...
// Simple function to parse moves like "e2e4"
Move parseMove(const Board& board, const std::string& moveStr) {
    ...
    for (const Move& move : legalMoves) {
        if (::seajay::from(move) == from && ::seajay::to(move) == to) {
            return move;
        }
    }
...
int main() {
    ...
    for (const std::string& moveStr : moves) {
        if (moveStr.empty()) break; // End of moves
    ...
    std::cout << "Final position after move " << moveCount << ":\n";
    ...
    // Find checking pieces - manually since getCheckers is private
    if (whiteInCheck) {
        std::cout << "\nChecking pieces:\n";
        // Check all black pieces that attack the white king
        for (Square sq = A1; sq <= H8; sq = Square(sq + 1)) {
            ...
            if (piece != NO_PIECE && colorOf(piece) == BLACK) {
                ...
                if (type == QUEEN) {
                    // Queen at sq - check if it attacks white king
                    // For now, just report all queens
    ...
    std::cout << "Total legal king moves: " << kingMoveCount << "\n";
    ...
    // Additional check: verify all legal king moves are safe
    std::cout << "\nVerifying safety of all legal king moves:\n";
    for (const Move& move : legalMoves) {
        if (from(move) == whiteKing) {
            Square to = ::seajay::to(move);
            ...
            Board::UndoInfo undo;
            testBoard.makeMove(move, undo);
            
            bool stillInCheck = MoveGenerator::inCheck(testBoard, WHITE);
            testBoard.unmakeMove(move, undo);
            
            std::cout << "  " << squareToString(from(move)) << squareToString(to);
            std::cout << " - After move, in check: " << (stillInCheck ? "YES (BUG!)" : "NO");
            std::cout << "\n";
        }
    }
